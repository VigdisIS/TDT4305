...
def minHash(docs_signature_sets, hash_fn):
    ...
    signature_matrix = [
        [float("inf") for _ in range(num_cols)] for _ in range(num_hash_functions)
    ]
...
def candidates_similarities(candidate_docs, min_hash_matrix):
    ...
    # Calculate the similarity for each pair of documents
    for pair in candidate_docs:
        d1, d2 = pair

        # Calculate the fraction of hash functions on which the documents agree

        pair_sum = 0
        for i in range(num_hash_functions):
            if min_hash_matrix[i][d1] == min_hash_matrix[i][d2]:
                pair_sum += 1
